...
%>		
	...
	<hr/>
<%
	List<ReplyDTO> replyList = dao.selectReply(b_idx);
	for(ReplyDTO r : replyList){
%>
		<p><%=r.getUserid()%> : <%=r.getContent()%> ( <%=r.getRegdate()%> ) 
<%
		if(r.getUserid().equals((String)session.getAttribute("userid"))){
%>
		<input type="button" value="삭제" onclick="location.href='reply_del.jsp?re_idx=<%=r.getIdx()%>&b_idx=<%=b_idx%>'">
<%
		}
%>		</p>
<%
